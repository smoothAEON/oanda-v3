...
from core.models import TimeAlert, normalize_time_alert_local_time
...
def next_fixed_datetime_fire_at(
    ...
    *,
    ...
    """Return the UTC fire time for one exact local YYYY-MM-DD HH:MM reminder."""

    now = now_utc or datetime.now(timezone.utc)
    zone = ZoneInfo(timezone_name)
    normalized = normalize_time_alert_local_time(local_datetime_text)
    if " " not in normalized:
        raise ValueError("Time must use YYYY-MM-DD HH:MM in 24-hour format.")
    try:
        parsed = datetime.strptime(normalized, "%Y-%m-%d %H:%M")
    except ValueError as exc:
        raise ValueError("Time must use YYYY-MM-DD HH:MM in 24-hour format.") from exc
    candidate = parsed.replace(tzinfo=zone)
    if candidate <= now.astimezone(zone):
        raise ValueError("Datetime is in the past; provide a future date and time.")
    return candidate.astimezone(timezone.utc)


def next_fixed_alert_fire_at(
    local_time_text: str,
    *,
    now_utc: datetime | None = None,
    timezone_name: str = DEFAULT_TIME_ALERT_TIMEZONE,
) -> datetime:
    """Return the next UTC fire time for one fixed alert definition."""

    if " " in str(local_time_text).strip():
        return next_fixed_datetime_fire_at(
            local_time_text,
            now_utc=now_utc,
                ...
                self.alert_repository.insert_alert_history(
                    {
                        ...
                        "granularity": None,
                        "indicator": None,
                        "triggered_at": updated.last_fired_at or now,
                        "trigger_value": updated.session_name or updated.local_time,
                        ...
                        "trigger_context": {
                            ...
                            "next_fire_at": None if updated.next_fire_at is None else updated.next_fire_at.isoformat(),
                        },
                    }
                )
