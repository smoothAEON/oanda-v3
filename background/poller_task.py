...
class TradePollerTask:
    ...
    def _poll_once_sync(self) -> list[object]:
        ...
        current_instruments = {
            ...
            for payload in current_by_id.values()
            if payload.get("instrument") is not None
        }
        existing_open = {trade.trade_id: trade for trade in self.trade_repository.list_open()}
        account_currency = (
            self._safe_account_currency()
            if current_by_id or existing_open
            else self.settings.account_currency
        )
        emitted: list[object] = []
    ...
    def _infer_close_reason(
        ...
        return infer_close_reason(
            close_price=close_price,
            sl_price=sl_price,
            tp_price=tp_price,
            raw_reason=raw_reason,
            evidence_sources=transactions or (),
        )
    ...
    def _safe_account_currency(self) -> str:
        ...
        try:
            ...
            log_failure(
                self.logger,
                "trade_poller_account_summary_unavailable",
                exc,
                level="warning",
            )
            return self.settings.account_currency
    ...
    def _dispatch_open_trade_instruments(self, instruments: set[str]) -> None:
        if self.open_trade_instruments_handler is None:
            return
        try:
            self.open_trade_instruments_handler(set(instruments))
        except Exception as exc:
            log_failure(
                self.logger,
                "trade_poller_open_trade_watch_update_failed",
                exc,
                level="warning",
