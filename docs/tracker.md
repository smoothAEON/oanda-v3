...
| Time alerts and session reminders | `shipped` | `/timealert`, `/listtimealerts`, `/cleartimealert`, `/exporttimealerts`, and `/importtimealerts` are implemented. Alerts persist in UTC, accept SGT `HH:MM` and exact `YYYY-MM-DD HH:MM` input, support London, New York, and market-open session reminders, and evaluate every minute. |
