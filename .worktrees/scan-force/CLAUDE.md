...
Current shipped behavior: multi-timeframe scan pipeline (M15/H1/H4/D), SMC summaries, TA-Lib/pandas-ta indicators, spread and chop gates, SFP, Turtle Soup, ORB, HTF bias, Telegram bot with auth/account/analysis/chart/journal/alert commands, trade journaling with MAE/MFE, price and indicator alert evaluation with Telegram push delivery, trade-open and trade-close push delivery, CSV candle cache with TinyDB metadata.
...
- `tolerance`, `spread`, `chop`, `chart`, `chart_mode`, `scan_interval`, `trade_push`, `session_alerts`
