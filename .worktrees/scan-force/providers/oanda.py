...
class OandaMarketDataProvider:
    ...
    def get_current_price(self, instrument: str) -> PriceSnapshot:
        ...
        payload = self._request_pricing_payload(instrument)
        ...
        if not prices:
            ...
            raise RuntimeError(f"OANDA pricing response for {instrument} contained no prices.")
        ...
        try:
            ...
            raise RuntimeError(
                f"OANDA pricing response for {instrument} is missing bid/ask quotes."
            ) from exc
    ...
    def get_candle_freshness(self, instrument: str, timeframe: str) -> CandleFreshness:
        ...
        get_instrument_spec(instrument)
        get_timeframe_delta(timeframe)
        return self.cache.get_candle_freshness(instrument, timeframe)
    ...
    def get_cached_candles(
        ...
        if resolved_count <= 0:
            ...
            raise ValueError("count must be a positive integer.")
        return self.cache.get_cached_candles(instrument, timeframe, resolved_count)
