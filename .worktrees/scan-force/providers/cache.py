...
class CandleCache:
    ...
    def get_candles(
        ...
        if memory_entry is not None:
            self._log_cache_lookup("memory", instrument, timeframe, memory_entry)
            if is_cache_fresh(memory_entry.last_completed_candle, timeframe):
                ...
                return self._replace_from_api(instrument, timeframe, count, fetcher)
            return self._append_refresh(instrument, timeframe, count, memory_entry, fetcher)

        self._log_cache_lookup("memory", instrument, timeframe, None)
        csv_entry = self._load_csv_entry(instrument, timeframe)
        if csv_entry is not None:
            self._log_cache_lookup("csv", instrument, timeframe, csv_entry)
            with self._lock:
            ...
            if is_cache_fresh(csv_entry.last_completed_candle, timeframe):
                ...
                return self._replace_from_api(instrument, timeframe, count, fetcher)
            return self._append_refresh(instrument, timeframe, count, csv_entry, fetcher)

        self._log_cache_lookup("csv", instrument, timeframe, None)
        return self._replace_from_api(instrument, timeframe, count, fetcher)
    ...
    def get_candle_freshness(self, instrument: str, timeframe: str) -> CandleFreshness:
        ...
        with self._lock:
            memory_entry = self._memory_cache.get(key)
        if memory_entry is not None:
            return self._freshness_from_entry(instrument, timeframe, memory_entry)

        metadata = self.trade_store.get_cache_metadata(instrument, timeframe)
        if metadata is not None:
            return self._freshness_from_metadata(instrument, timeframe, metadata)
