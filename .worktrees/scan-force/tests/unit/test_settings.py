...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("RUPTURES_PENALTY", "0"),
        ("HTF_BIAS_WEIGHT_D", "0"),
        ("HTF_BIAS_WEIGHT_H4", "-0.1"),
        ("HTF_BIAS_WEIGHT_H1", "0"),
        ("HTF_BIAS_NEUTRAL_BAND", "1"),
        ("HTF_TRANSITION_WINDOW_D", "0"),
        ("HTF_TRANSITION_WINDOW_H4", "-1"),
        ("HTF_TRANSITION_WINDOW_H1", "0"),
        ("SCAN_INTERVAL_MINUTES", "-5"),
...
def test_load_settings_uses_trade_helper_runtime_defaults(
    ...
    assert settings.htf_bias_weight_d == pytest.approx(0.50)
    assert settings.htf_bias_weight_h4 == pytest.approx(0.30)
    assert settings.htf_bias_weight_h1 == pytest.approx(0.20)
    assert settings.htf_bias_neutral_band == pytest.approx(0.15)
    assert settings.htf_transition_window_d == 3
    assert settings.htf_transition_window_h4 == 4
    assert settings.htf_transition_window_h1 == 6
