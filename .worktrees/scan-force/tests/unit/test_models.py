...
def build_spread(instrument: str = "EUR_USD") -> SpreadResult:
    ...
    return SpreadResult(
        instrument=instrument,
        raw_spread=spec.pip_size * spread_pips,
...
def test_timeframe_snapshot_serializes_and_is_frozen() -> None:
    ...
    assert dumped["sfp"]["detected"] is False
    assert dumped["turtle_soup"]["detected"] is False

    with pytest.raises(ValidationError):
        snapshot.version = 99  # type: ignore[misc]


def test_active_zone_summary_rejects_more_than_ten_order_blocks() -> None:
    block = OrderBlockSummary(
        direction="BULLISH",
        upper_price=1.1020,
        lower_price=1.1010,
    ...
    with pytest.raises(ValidationError):
        ActiveZoneSummary(order_blocks=(block,) * 11)
...
def test_stage_08_pattern_models_validate_contracts() -> None:
    sfp = SFPResult(
        detected=True,
        direction="BEARISH",
        reference_level=1.1050,
        reference_time=BASE_TIME - timedelta(hours=2),
        sweep_price=1.1060,
        close_price=1.1040,
        occurred_at=BASE_TIME,
    )
    turtle = TurtleSoupResult(
        detected=True,
        direction="BULLISH",
        reference_level=1.0950,
    ...
    assert sfp.reference_level == 1.1050
    assert turtle.lookback_bars == 20

    with pytest.raises(ValidationError):
        SFPResult(detected=True, direction="BULLISH")

    with pytest.raises(ValidationError):
        TurtleSoupResult(detected=False, lookback_bars=20)

    with pytest.raises(ValidationError):
        ORBResult(detected=False, session="LONDON")
...
def test_timeframe_snapshot_rejects_orb_on_non_m15_and_accepts_m15() -> None:
    orb = ORBResult(
        detected=True,
        direction="BULLISH",
        session="LONDON",
    ...
    with pytest.raises(ValidationError):
        TimeframeSnapshot(
            ...
            last_completed_candle=BASE_TIME,
            computed_at=BASE_TIME + timedelta(minutes=1),
            candle_range_start=BASE_TIME - get_timeframe_delta("H1"),
    ...
    m15_snapshot = TimeframeSnapshot(
        ...
        last_completed_candle=BASE_TIME,
        computed_at=BASE_TIME + timedelta(minutes=1),
        candle_range_start=BASE_TIME - get_timeframe_delta("M15"),
        candle_range_end=BASE_TIME,
        indicators=IndicatorValueSummary(),
        structure=StructureEventSummary(),
        zones=ActiveZoneSummary(),
        ...
        chop=build_chop(),
        orb=orb,
        freshness=build_freshness(timeframe="M15"),
    )

    assert m15_snapshot.orb == orb


def test_instrument_bundle_rejects_mismatched_member_keys_and_hidden_staleness() -> None:
    freshness = build_freshness()
    ...
    stale = build_freshness(is_fresh=False, staleness_seconds=3600.0)
    with pytest.raises(ValidationError):
        InstrumentBundle(
            instrument="EUR_USD",
            created_at=BASE_TIME + timedelta(minutes=2),
            ...
            calendar_version=0,
            mixed_freshness=False,
            stalest_timeframe="H1",
            stalest_age_seconds=3600.0,
            member_freshness={"H1": stale},
        )
...
def test_neutral_placeholder_models_are_valid() -> None:
    ...
    bundle = InstrumentBundle(
        ...
        stalest_timeframe=None,
        stalest_age_seconds=0.0,
        member_freshness={"H1": snapshot.freshness},
    )

    assert snapshot.indicators.metrics == ()
    assert bundle.htf_bias.direction == "NEUTRAL"
    assert bundle.member_freshness["H1"].is_fresh is True
    assert snapshot.sfp == SFPResult()
    assert snapshot.turtle_soup == TurtleSoupResult()
...
def test_fib_summary_and_trade_plan_contracts_are_bounded() -> None:
    fib = FibSummary(
        instrument="EUR_USD",
        timeframe="H1",
        direction="BULLISH",
        ...
        anchor_low=1.1000,
        current_price=None,
        levels=(FibLevel(label="50.0%", ratio=0.5, price=1.1100),),
        as_of=BASE_TIME,
    )
    trade_plan = TradePlanSummary(
        ...
        target_price=1.1080,
        reward_risk=3.5,
        rationale=("HTF bias BULLISH.",),
        computed_at=BASE_TIME,
    )

    assert fib.levels[0].label == "50.0%"
    assert trade_plan.valid is True
