...
# ---------------------------------------------------------------------------
# 1. Full scan cycle: all 12 instruments through the orchestrator
# ---------------------------------------------------------------------------
...
class TestScanOrchestratorFullCycle:
    ...
    def test_scan_all_produces_complete_cycle(
        ...
        """scan_all() should scan all 12 instruments, publish 48 snapshots and 12 bundles."""

        status: ScanCycleStatus = scan_orchestrator.scan_all()

        # Basic counters
        assert len(status.scanned_instruments) == 12, (
            f"Expected 12 scanned instruments, got {len(status.scanned_instruments)}"
        )
        assert status.snapshots_published == 48, (
            f"Expected 48 snapshots (12x4), got {status.snapshots_published}"
        )
        assert status.bundles_published == 12, (
            f"Expected 12 bundles, got {status.bundles_published}"
        )
...
class TestCacheWarmerPopulatesAllInstruments:
    """CacheWarmer should populate 48 CSV files and be faster on the second run."""

    @pytest.mark.timeout(300)
    def test_warm_all_creates_csv_files_and_second_run_is_faster(
        self, live_provider, always_open_market_hours
    ) -> None:
        """warm_all() should return 48, create CSV files, and the second run should be at least 2x faster."""
        ...
        assert result_1 == 48, f"Expected 48 timeframe slots warmed, got {result_1}"
        ...
        assert result_2 == 48, f"Second warm_all() returned {result_2}, expected 48"
...
class TestFullRuntimeLifecycle:
    ...
    async def test_full_lifecycle(
        ...
        assert scan_status.bundles_published == 12
