from __future__ import annotations

from datetime import datetime, timedelta, timezone
from pathlib import Path
from types import SimpleNamespace

import pytest

from alerts.alert_repository import AlertRepository
from bot import bot as bot_module
from bot.security_manager import SecurityManager
from config.settings import load_settings
from core.enums import IndicatorKind, TradeState
from core.models import (
    BotSessionRecord,
    CalendarEvent,
    CalendarRefreshStatus,
    CloseReason,
    ExcursionSample,
    MacroContextStatus,
    MacroIndicatorStatus,
    MarketHoursOverview,
    MarketHoursStatus,
    ORBResult,
    SFPResult,
    TradeRecord,
    TurtleSoupResult,
)
from data.persistence.trade_store import TradeStore
from orchestration.scan_orchestrator import ScanOrchestrator
from tests.integration.test_scan_orchestrator import (
    ClosedMarketHours,
    StubCalendarProvider as CacheOnlyCalendarProvider,
    StubHTFBiasAnalyzer,
    StubMarketDataProvider,
    StubSmcAdapter,
    build_chop,
    build_indicator_summary,
    build_spread,
)


BASE_TIME = datetime(2026, 3, 22, 8, 0, tzinfo=timezone.utc)


def write_env_file(path: Path, *, tinydb_path: Path) -> Path:
    path.write_text(
        "\n".join(
            (
                "OANDA_API_KEY=api-key",
                "OANDA_ACCOUNT_ID=account-id",
                "OANDA_ENVIRONMENT=practice",
                "TELEGRAM_BOT_TOKEN=telegram-token",
                "TELEGRAM_CHAT_ID=123456789",
                "TELEGRAM_BOT_PASSWORD=bot-password",
                "TELEGRAM_ADMIN_IDS=111,222",
                f"TINYDB_PATH={tinydb_path.as_posix()}",
            )
        ),
        encoding="utf-8",
    )
    return path


def build_settings_and_store(tmp_path: Path):
    env_file = write_env_file(tmp_path / ".env", tinydb_path=tmp_path / "bot.json")
    settings = load_settings(env_file=env_file)
    store = TradeStore(db_path=settings.tinydb_path, settings=settings)
    return settings, store


class RecorderMessage:
    def __init__(self) -> None:
        self.texts: list[str] = []

    async def reply_text(self, text: str, **kwargs) -> None:  # type: ignore[no-untyped-def]
        self.texts.append(text)


def build_update(user_id: int = 111, chat_id: int = 222) -> SimpleNamespace:
    return SimpleNamespace(
        effective_message=RecorderMessage(),
        effective_user=SimpleNamespace(id=user_id, username="tester", first_name="Test"),
        effective_chat=SimpleNamespace(id=chat_id),
    )


class StubMarketState:
    def __init__(self, *, bundle=None, snapshot=None) -> None:  # type: ignore[no-untyped-def]
        self.bundle = bundle
        self.snapshot = snapshot
        self.bundle_calls = 0
        self.snapshot_calls = 0

    def get_bundle(self, instrument: str):
        self.bundle_calls += 1
        return self.bundle

    def get_snapshot(self, instrument: str, timeframe: str):
        self.snapshot_calls += 1
        return self.snapshot


class StubCalendarProvider:
    def __init__(self) -> None:
        self.calls = 0

    def filter_events(
        self,
        *,
        impacts=None,
        window_start=None,
        window_end=None,
        currencies=None,
        countries=None,
    ):
        self.calls += 1
        return (
            CalendarEvent(
                event_time=BASE_TIME,
                currency="USD",
                title="CPI",
                impact="HIGH",
                forecast=None,
                previous=None,
                actual=None,
                is_blackout=True,
            ),
        )


class StubScanOrchestrator:
    def __init__(self, *, bundle=None, snapshot=None) -> None:  # type: ignore[no-untyped-def]
        self.bundle = bundle
        self.snapshot = snapshot
        self.refresh_instrument_calls: list[str] = []
        self.refresh_snapshot_calls: list[tuple[str, str]] = []
        self.scan_calls = 0
        self.scan_force_calls: list[bool] = []
        self.refresh_force_calls: list[tuple[str, bool]] = []
        self.last_scan_status = SimpleNamespace(
            run_kind="full",
            scanned_instruments=(),
            bundles_published=0,
            snapshots_published=0,
            errors=(),
        )
        self.market_hours_status = None
        self.macro_status = MacroContextStatus()
        self.calendar_status = CalendarRefreshStatus()
        self.calendar_provider = StubCalendarProvider()
        self.market_hours_service = SimpleNamespace(
            get_status=lambda: SimpleNamespace(
                is_market_open=True,
                reason="open",
                next_open_at=None,
                next_close_at=BASE_TIME,
            )
        )

    def refresh_instrument(self, instrument: str, *, force: bool = False):
        self.refresh_instrument_calls.append(instrument)
        self.refresh_force_calls.append((instrument, force))
        self.last_scan_status = SimpleNamespace(
            run_kind="instrument_refresh",
            scanned_instruments=(instrument,),
            bundles_published=1 if self.bundle is not None else 0,
            snapshots_published=4 if self.bundle is not None else 0,
            errors=(),
        )
        return self.bundle

    def refresh_snapshot(self, instrument: str, timeframe: str):
        self.refresh_snapshot_calls.append((instrument, timeframe))
        self.last_scan_status = SimpleNamespace(
            run_kind="snapshot_refresh",
            scanned_instruments=(instrument,),
            bundles_published=0,
            snapshots_published=1 if self.snapshot is not None else 0,
            errors=(),
        )
        return self.snapshot

    def scan_all(self, *, force: bool = False):
        self.scan_calls += 1
        self.scan_force_calls.append(force)
        self.last_scan_status = SimpleNamespace(
            run_kind="full",
            scanned_instruments=("EUR_USD", "XAU_USD"),
            bundles_published=2,
            snapshots_published=8,
            errors=("EUR_USD: none",),
        )
        return self.last_scan_status

    def refresh_calendar(self, *, force: bool = True):
        self.calendar_status = CalendarRefreshStatus(
            last_attempted_at=BASE_TIME,
            last_refreshed_at=BASE_TIME,
            calendar_version=1,
            event_count=1,
            next_high_impact=BASE_TIME,
            used_cached=False,
            last_error=None,
        )
        return self.calendar_status

    def refresh_macro(self, *, force: bool = True):
        return self.macro_status


class StubAccountClient:
    def __init__(self) -> None:
        self.summary_calls = 0
        self.position_calls = 0
        self.order_calls = 0

    async def get_account_summary(self):
        self.summary_calls += 1
        return SimpleNamespace(
            account_id="001-001",
            environment="practice",
            currency="USD",
            balance=1000.0,
            nav=1010.0,
            unrealized_pl=10.0,
            realized_pl=5.0,
            margin_used=12.0,
            margin_available=998.0,
            open_trade_count=1,
            open_position_count=1,
            pending_order_count=2,
        )

    async def get_open_positions(self):
        self.position_calls += 1
        return [
            SimpleNamespace(
                trade_id="trade-1",
                instrument="EUR_USD",
                units=1.0,
                open_price=1.1000,
                unrealized_pl=2.5,
                realized_pl=0.0,
                stop_loss_price=1.0900,
                take_profit_price=1.1200,
                gslo_price=None,
                opened_at=BASE_TIME,
                direction="LONG",
            )
        ]

    async def get_open_orders(self):
        self.order_calls += 1
        return [
            SimpleNamespace(
                order_id="order-1",
                instrument="EUR_USD",
                order_type="LIMIT",
                direction="LONG",
                price=1.1050,
                state="PENDING",
            )
        ]

    async def get_pricing(self, instrument: str):
        return SimpleNamespace(
            instrument=instrument,
            bid=1.1000,
            ask=1.1002,
            spread_pips=2.0,
            fetched_at=BASE_TIME,
        )


class StubRuntime:
    def __init__(self, scan_orchestrator) -> None:  # type: ignore[no-untyped-def]
        self.scan_orchestrator = scan_orchestrator
        self.stream_task = SimpleNamespace(stream_status=lambda: SimpleNamespace(state="RUNNING", reconnect_count=0, last_tick_at=BASE_TIME))


@pytest.mark.asyncio
async def test_bias_uses_bundle_before_refresh(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    bundle = SimpleNamespace(
        instrument="EUR_USD",
        htf_bias=SimpleNamespace(
            direction="BULLISH",
            alignment_score=0.75,
            timeframe_votes={"D": "BULLISH", "H4": "BULLISH", "H1": "BULLISH"},
        ),
        mixed_freshness=False,
    )
    market_state = StubMarketState(bundle=bundle)
    orchestrator = StubScanOrchestrator(bundle=SimpleNamespace(ignored=True))
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: StubRuntime(orchestrator),
        bot_module.SECURITY_MANAGER_KEY: security,
        bot_module.MARKET_STATE_KEY: market_state,
        bot_module.SCAN_ORCHESTRATOR_KEY: orchestrator,
    }
    update = build_update()

    await bot_module.bias_command(update, SimpleNamespace(bot_data=bot_data, args=["EUR_USD"]))

    assert market_state.bundle_calls == 1
    assert orchestrator.refresh_instrument_calls == []
    assert "Bias EUR_USD" in update.effective_message.texts[-1]
    assert "Direction: BULLISH" in update.effective_message.texts[-1]


@pytest.mark.asyncio
async def test_smc_uses_snapshot_before_refresh(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    snapshot = SimpleNamespace(
        instrument="EUR_USD",
        timeframe="H1",
        freshness=SimpleNamespace(is_fresh=True),
        structure=SimpleNamespace(recent_breaks=(), latest_break=None),
        zones=SimpleNamespace(order_blocks=()),
        liquidity=SimpleNamespace(levels=()),
        chop=SimpleNamespace(status="PASS"),
        spread=SimpleNamespace(is_acceptable=True),
    )
    market_state = StubMarketState(snapshot=snapshot)
    orchestrator = StubScanOrchestrator(snapshot=SimpleNamespace(ignored=True))
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: StubRuntime(orchestrator),
        bot_module.SECURITY_MANAGER_KEY: security,
        bot_module.MARKET_STATE_KEY: market_state,
        bot_module.SCAN_ORCHESTRATOR_KEY: orchestrator,
    }
    update = build_update()

    await bot_module.smc_command(
        update,
        SimpleNamespace(bot_data=bot_data, args=["EUR_USD", "H1"]),
    )

    assert market_state.snapshot_calls == 1
    assert orchestrator.refresh_snapshot_calls == []
    assert "SMC EUR_USD H1" in update.effective_message.texts[-1]


@pytest.mark.asyncio
async def test_smc_surfaces_stale_snapshot_warning_without_refresh(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    stale_snapshot = SimpleNamespace(
        instrument="EUR_USD",
        timeframe="H1",
        last_completed_candle=BASE_TIME,
        freshness=SimpleNamespace(
            is_fresh=False,
            last_completed_candle=BASE_TIME,
            staleness_seconds=3600.0,
        ),
        structure=SimpleNamespace(recent_breaks=(), latest_break=None),
        zones=SimpleNamespace(order_blocks=()),
        liquidity=SimpleNamespace(levels=()),
        chop=SimpleNamespace(status="PASS"),
        spread=SimpleNamespace(is_acceptable=True),
    )
    market_state = StubMarketState(snapshot=stale_snapshot)
    orchestrator = StubScanOrchestrator(snapshot=stale_snapshot)
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: StubRuntime(orchestrator),
        bot_module.SECURITY_MANAGER_KEY: security,
        bot_module.MARKET_STATE_KEY: market_state,
        bot_module.SCAN_ORCHESTRATOR_KEY: orchestrator,
    }
    update = build_update()

    await bot_module.smc_command(
        update,
        SimpleNamespace(bot_data=bot_data, args=["EUR_USD", "H1"]),
    )

    assert orchestrator.refresh_snapshot_calls == []
    assert "Warning: snapshot is stale" in update.effective_message.texts[-1]


@pytest.mark.asyncio
async def test_async_account_commands_and_scan_paths(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    account_client = StubAccountClient()
    orchestrator = StubScanOrchestrator(
        bundle=SimpleNamespace(instrument="EUR_USD"),
        snapshot=SimpleNamespace(instrument="EUR_USD"),
    )
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: StubRuntime(orchestrator),
        bot_module.SECURITY_MANAGER_KEY: security,
        bot_module.ACCOUNT_CLIENT_KEY: account_client,
        bot_module.SETTINGS_KEY: settings,
        bot_module.SCAN_ORCHESTRATOR_KEY: orchestrator,
        bot_module.MARKET_STATE_KEY: StubMarketState(
            bundle=SimpleNamespace(instrument="EUR_USD"),
            snapshot=SimpleNamespace(freshness=SimpleNamespace(is_fresh=True)),
        ),
    }

    account_update = build_update()
    positions_update = build_update()
    orders_update = build_update()
    scan_update = build_update()
    scan_one_update = build_update()

    await bot_module.account_command(account_update, SimpleNamespace(bot_data=bot_data, args=[]))
    await bot_module.positions_command(positions_update, SimpleNamespace(bot_data=bot_data, args=[]))
    await bot_module.orders_command(orders_update, SimpleNamespace(bot_data=bot_data, args=[]))
    await bot_module.scan_command(scan_update, SimpleNamespace(bot_data=bot_data, args=[]))
    await bot_module.scan_command(
        scan_one_update,
        SimpleNamespace(bot_data=bot_data, args=["EUR_USD"]),
    )

    assert account_client.summary_calls == 1
    assert account_client.position_calls == 1
    assert account_client.order_calls == 1
    assert orchestrator.scan_calls == 1
    assert orchestrator.refresh_instrument_calls == ["EUR_USD"]
    assert "Account 001-001 (practice)" in account_update.effective_message.texts[-1]
    assert "Open Trades" in positions_update.effective_message.texts[-1]
    assert "pnl=+2.50 USD" in positions_update.effective_message.texts[-1]
    assert "Open Orders" in orders_update.effective_message.texts[-1]
    assert "Full scan complete" in scan_update.effective_message.texts[-1]
    assert "Scan complete for EUR_USD" in scan_one_update.effective_message.texts[-1]


@pytest.mark.asyncio
async def test_scan_then_pdl_uses_closed_market_cache_without_seed_snapshot(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    orchestrator = ScanOrchestrator(
        settings=settings,
        market_data_provider=StubMarketDataProvider(cached_available=True),
        calendar_provider=CacheOnlyCalendarProvider(),
        market_hours_service=ClosedMarketHours(),
        smc_adapter=StubSmcAdapter(),
        htf_bias_analyzer=StubHTFBiasAnalyzer(),
        indicator_builder=build_indicator_summary,
        spread_evaluator=build_spread,
        chop_evaluator=build_chop,
        sfp_detector=lambda candles, timeframe, swing_length=50: SFPResult(),
        turtle_soup_detector=lambda candles, timeframe: TurtleSoupResult(),
        orb_detector=lambda candles, timeframe: ORBResult(),
    )
    bot_data = {
        bot_module.SECURITY_MANAGER_KEY: security,
        bot_module.SCAN_ORCHESTRATOR_KEY: orchestrator,
        bot_module.MARKET_STATE_KEY: orchestrator.market_state,
    }
    scan_update = build_update()
    pdl_update = build_update()

    try:
        await bot_module.scan_command(
            scan_update,
            SimpleNamespace(bot_data=bot_data, args=["XAU_USD"]),
        )
        await bot_module.pdl_command(
            pdl_update,
            SimpleNamespace(bot_data=bot_data, args=["XAU_USD"]),
        )
    finally:
        store.close()

    assert "Scan complete for XAU_USD" in scan_update.effective_message.texts[-1]
    assert "Data unavailable for XAU_USD H1. Try /scan first." not in pdl_update.effective_message.texts[-1]
    assert "PDL XAU_USD" in pdl_update.effective_message.texts[-1]


@pytest.mark.asyncio
async def test_calendar_prefers_cached_status_then_refreshes_force_false(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    orchestrator = StubScanOrchestrator()
    runtime = StubRuntime(orchestrator)
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: runtime,
        bot_module.SECURITY_MANAGER_KEY: security,
    }
    update = build_update()

    await bot_module.calendar_command(update, SimpleNamespace(bot_data=bot_data, args=["today"]))

    assert orchestrator.calendar_provider.calls == 1
    assert orchestrator.calendar_status.calendar_version == 1
    assert "Calendar (SGT)" in update.effective_message.texts[-1]
    assert "USD (1)" in update.effective_message.texts[-1]
    assert "CPI" in update.effective_message.texts[-1]


@pytest.mark.asyncio
async def test_marketstatus_renders_category_and_macro_lines(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    orchestrator = StubScanOrchestrator()
    orchestrator.market_hours_status = MarketHoursOverview(
        overall=MarketHoursStatus(
            checked_at=BASE_TIME,
            is_market_open=True,
            source="test",
            category="overall",
            reason="partial_open",
            next_open_at=None,
            next_close_at=BASE_TIME + timedelta(hours=5),
        ),
        fx=MarketHoursStatus(
            checked_at=BASE_TIME,
            is_market_open=True,
            source="test",
            category="fx",
            reason="open",
            next_open_at=None,
            next_close_at=BASE_TIME + timedelta(hours=5),
        ),
        metals=MarketHoursStatus(
            checked_at=BASE_TIME,
            is_market_open=False,
            source="test",
            category="metals",
            reason="holiday_closed",
            next_open_at=BASE_TIME + timedelta(days=1),
            next_close_at=None,
        ),
    )
    orchestrator.macro_status = MacroContextStatus(
        last_attempted_at=BASE_TIME,
        last_refreshed_at=BASE_TIME,
        used_cached=True,
        last_error="macro backend unavailable",
        vix=MacroIndicatorStatus(name="VIX", symbol="^VIX", value=31.05, as_of=BASE_TIME),
        dxy=MacroIndicatorStatus(name="DXY", symbol="DX-Y.NYB", value=100.15, as_of=BASE_TIME),
    )
    runtime = StubRuntime(orchestrator)
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: runtime,
        bot_module.SECURITY_MANAGER_KEY: security,
    }
    update = build_update()

    await bot_module.marketstatus_command(update, SimpleNamespace(bot_data=bot_data, args=[]))

    text = update.effective_message.texts[-1]
    assert "Overall: open" in text
    assert "FX: open" in text
    assert "Metals: closed" in text
    assert "Macro source: cached" in text
    assert "VIX: 31.05000" in text
    assert "DXY: 100.15000" in text
    assert "Macro note: macro backend unavailable" in text


@pytest.mark.asyncio
async def test_help_rejects_session_from_different_chat(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    update = build_update(user_id=111, chat_id=333)

    try:
        await bot_module.help_command(
            update,
            SimpleNamespace(bot_data={bot_module.SECURITY_MANAGER_KEY: security}, args=[]),
        )
    finally:
        store.close()

    assert update.effective_message.texts[-1] == bot_module.CHAT_SCOPE_TEXT


@pytest.mark.asyncio
async def test_price_alert_list_and_clear_are_chat_scoped(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    repository = AlertRepository(store=store)
    security.authenticate(user_id=111, chat_id=222, password="bot-password")
    security.authenticate(user_id=222, chat_id=333, password="bot-password")

    try:
        alert_a = repository.upsert_price_alert(
            {
                "instrument": "XAU_USD",
                "target_price": 3050.0,
                "direction": "above",
                "chat_id": 222,
                "created_at": BASE_TIME,
            }
        )
        alert_b = repository.upsert_price_alert(
            {
                "instrument": "EUR_USD",
                "target_price": 1.0500,
                "direction": "below",
                "chat_id": 333,
                "created_at": BASE_TIME,
            }
        )

        bot_data = {
            bot_module.SECURITY_MANAGER_KEY: security,
            bot_module.ALERT_REPOSITORY_KEY: repository,
        }
        update_a = build_update(user_id=111, chat_id=222)
        update_b = build_update(user_id=222, chat_id=333)

        await bot_module.list_price_alerts_command(update_a, SimpleNamespace(bot_data=bot_data, args=[]))
        await bot_module.list_price_alerts_command(update_b, SimpleNamespace(bot_data=bot_data, args=[]))

        assert f"#{alert_a.id}" in update_a.effective_message.texts[-1]
        assert f"#{alert_b.id}" not in update_a.effective_message.texts[-1]
        assert f"#{alert_b.id}" in update_b.effective_message.texts[-1]
        assert f"#{alert_a.id}" not in update_b.effective_message.texts[-1]

        forbidden_clear = build_update(user_id=111, chat_id=222)
        await bot_module.clear_price_alert_command(
            forbidden_clear,
            SimpleNamespace(bot_data=bot_data, args=[str(alert_b.id)]),
        )
        assert forbidden_clear.effective_message.texts[-1] == f"Price alert {alert_b.id} not found."

        allowed_clear = build_update(user_id=222, chat_id=333)
        await bot_module.clear_price_alert_command(
            allowed_clear,
            SimpleNamespace(bot_data=bot_data, args=[str(alert_b.id)]),
        )
        assert allowed_clear.effective_message.texts[-1] == f"Price alert {alert_b.id} cleared."
    finally:
        store.close()


@pytest.mark.asyncio
async def test_indicator_alert_list_and_clear_are_chat_scoped(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    repository = AlertRepository(store=store)
    security.authenticate(user_id=111, chat_id=222, password="bot-password")
    security.authenticate(user_id=222, chat_id=333, password="bot-password")

    try:
        alert_a = store.upsert_indicator_alert(
            {
                "instrument": "XAU_USD",
                "granularity": "H1",
                "indicator": IndicatorKind.RSI,
                "condition": "below",
                "threshold": 30.0,
                "chat_id": 222,
                "created_at": BASE_TIME,
            }
        )
        alert_b = store.upsert_indicator_alert(
            {
                "instrument": "EUR_USD",
                "granularity": "H1",
                "indicator": IndicatorKind.RSI,
                "condition": "below",
                "threshold": 30.0,
                "chat_id": 333,
                "created_at": BASE_TIME,
            }
        )

        bot_data = {
            bot_module.SECURITY_MANAGER_KEY: security,
            bot_module.ALERT_REPOSITORY_KEY: repository,
        }
        update_a = build_update(user_id=111, chat_id=222)
        update_b = build_update(user_id=222, chat_id=333)

        await bot_module.list_indicator_alerts_command(update_a, SimpleNamespace(bot_data=bot_data, args=[]))
        await bot_module.list_indicator_alerts_command(update_b, SimpleNamespace(bot_data=bot_data, args=[]))

        assert f"#{alert_a.id}" in update_a.effective_message.texts[-1]
        assert f"#{alert_b.id}" not in update_a.effective_message.texts[-1]
        assert f"#{alert_b.id}" in update_b.effective_message.texts[-1]
        assert f"#{alert_a.id}" not in update_b.effective_message.texts[-1]

        forbidden_clear = build_update(user_id=111, chat_id=222)
        await bot_module.clear_indicator_alert_command(
            forbidden_clear,
            SimpleNamespace(bot_data=bot_data, args=[str(alert_b.id)]),
        )
        assert forbidden_clear.effective_message.texts[-1] == f"Indicator alert {alert_b.id} not found."

        allowed_clear = build_update(user_id=222, chat_id=333)
        await bot_module.clear_indicator_alert_command(
            allowed_clear,
            SimpleNamespace(bot_data=bot_data, args=[str(alert_b.id)]),
        )
        assert allowed_clear.effective_message.texts[-1] == f"Indicator alert {alert_b.id} cleared."
    finally:
        store.close()


@pytest.mark.asyncio
async def test_scan_command_force_flag_is_parsed_and_forwarded(tmp_path: Path) -> None:
    settings, store = build_settings_and_store(tmp_path)
    security = SecurityManager(store=store, settings=settings)
    security.authenticate(
        user_id=111,
        chat_id=222,
        password="bot-password",
        username="tester",
        first_name="Test",
    )
    orchestrator = StubScanOrchestrator(bundle=SimpleNamespace(instrument="XAU_USD"))
    bot_data = {
        bot_module.BOT_RUNTIME_KEY: StubRuntime(orchestrator),
        bot_module.SECURITY_MANAGER_KEY: security,
        bot_module.SCAN_ORCHESTRATOR_KEY: orchestrator,
        bot_module.SETTINGS_KEY: settings,
    }

    full_force_update = build_update()
    single_force_update = build_update()
    single_no_force_update = build_update()

    # /scan force → scan_all(force=True)
    await bot_module.scan_command(
        full_force_update, SimpleNamespace(bot_data=bot_data, args=["force"])
    )
    # /scan xauusd force → refresh_instrument("XAU_USD", force=True)
    await bot_module.scan_command(
        single_force_update, SimpleNamespace(bot_data=bot_data, args=["xauusd", "force"])
    )
    # /scan xauusd → refresh_instrument("XAU_USD", force=False)
    await bot_module.scan_command(
        single_no_force_update, SimpleNamespace(bot_data=bot_data, args=["xauusd"])
    )

    assert orchestrator.scan_force_calls == [True]
    assert orchestrator.refresh_force_calls == [("XAU_USD", True), ("XAU_USD", False)]
    assert "Note: forced scan" in full_force_update.effective_message.texts[-1]
    assert "Note: forced scan" in single_force_update.effective_message.texts[-1]
    assert "Note: forced scan" not in single_no_force_update.effective_message.texts[-1]
    store.close()
