...
class ExcursionTracker:
    ...
    def _build_sample(self, trade: TradeRecord, tick: PriceTick) -> ExcursionSample:
        ...
        if trade.units > 0:
            adverse_pips = max(0.0, (trade.open_price - tick.bid) / spec.pip_size)
            favorable_pips = max(0.0, (tick.bid - trade.open_price) / spec.pip_size)
        else:
            adverse_pips = max(0.0, (tick.ask - trade.open_price) / spec.pip_size)
            favorable_pips = max(0.0, (trade.open_price - tick.ask) / spec.pip_size)
