...
- `tests/unit/test_time_alert_engine.py`
  Must prove fixed-time and session reminder scheduling, due-alert advancement, notification dispatch, and rescheduling semantics.
...
- `tests/unit/test_bot_commands.py`
  Must prove `/price --live`, `/tradeplan`, `/fib`, `/timealert`, `/listtimealerts`, `/cleartimealert`, and the Stage 16 runtime-config keys behave through the command layer without inline detector execution.
...
- `tests/integration/test_bot_trade_helper_commands.py`
  Must prove persistence-backed Stage 16 helper commands, including time-alert round trips, still operate through the authenticated chat scope and TinyDB-backed repositories.
...
- Time-alert flow
  Must prove `/timealert`, `/listtimealerts`, and `/cleartimealert` persist chat-scoped reminders, schedule them in UTC from SGT input, and dispatch notifications without coupling to market-open state.
