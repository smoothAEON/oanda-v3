...
def build_runtime(*, settings: Settings | None = None) -> BotRuntime:
    ...
    def spread_evaluator(price):
        result = evaluate_spread(price)
        threshold = runtime_config_manager.effective_spread_limit(result.max_spread_pips)
        if threshold == result.max_spread_pips:
            return result
        return result.model_copy(
            update={
                "max_spread_pips": threshold,
                "is_acceptable": result.spread_pips <= threshold,
            }
        )

    def chop_evaluator(indicators):
        return evaluate_chop(
            indicators,
            adx_threshold=runtime_config_manager.effective_chop_threshold(),
        )
