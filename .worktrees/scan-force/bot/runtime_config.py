...
class RuntimeConfigManager:
    ...
    def effective_chop_threshold(self) -> float:
        snapshot = self.snapshot()
        return DEFAULT_CHOP_THRESHOLD if snapshot.chop is None else snapshot.chop

    def effective_spread_limit(self, instrument_max_spread_pips: float) -> float:
        snapshot = self.snapshot()
        if snapshot.spread is None:
            return instrument_max_spread_pips
        return min(instrument_max_spread_pips, snapshot.spread)

    def effective_sr_tolerance(self) -> float | None:
        return self.snapshot().tolerance
...
def _normalize_config_value(
    ...
    try:
        ...
        raise ValueError(f"{key.value} config must be a positive number.") from exc
    if normalized_float <= 0:
        raise ValueError(f"{key.value} config must be a positive number.")
    return normalized_float


__all__ = [
    "DEFAULT_CHOP_THRESHOLD",
