...
from alerts.time_alert_engine import DEFAULT_TIME_ALERT_TIMEZONE, next_fixed_time_fire_at, next_session_fire_at
...
LIVE_PRICE_MAX_AGE_SECONDS = 30.0
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [instrument]
    if prefer_live:
        tick = _runtime(context).stream_task.latest_quote(
            instrument,
            max_age_seconds=LIVE_PRICE_MAX_AGE_SECONDS,
        )
        if tick is not None:
            ...
            lines.extend(
                [
                    ...
                    f"As of: {_format_optional_time(tick.time)}",
                    "Source: live stream",
                ]
            )
            await update.effective_message.reply_text("\n".join(lines))
    ...
    try:
        snapshot = await context.bot_data[ACCOUNT_CLIENT_KEY].get_pricing(instrument)
    ...
    lines.extend(
        [
            ...
            f"As of: {_format_optional_time(snapshot.fetched_at)}",
            "Source: REST pricing",
        ]
    )
    await update.effective_message.reply_text("\n".join(lines))
...
async def positions_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    def _fmt_level(level: float | None, mid: float | None, pip_size: float) -> str:
        if level is None:
            return "None"
        dist = f"({(level - mid) / pip_size:+.1f}p)" if mid is not None else ""
        return f"{level:.5f}{dist}"

    def _fmt_pnl(value: float | None, currency: str | None) -> str:
        if value is None:
            return "n/a"
        if currency is None:
            return f"{value:+.2f}"
    ...
    for position in positions:
        mid = mid_prices.get(position.instrument)
        pip_size = get_instrument_spec(position.instrument).pip_size
        settings = context.bot_data.get(SETTINGS_KEY)
        account_currency = getattr(position, "account_currency", None) or getattr(
            settings,
            "account_currency",
            None,
        )
        lines.append(
            f"#{position.trade_id} {position.instrument} {position.direction} units={position.units:.2f}"
            f" pnl={_fmt_pnl(position.unrealized_pl, account_currency)}"
            f" entry={_fmt_level(position.open_price, mid, pip_size)}"
            f" sl={_fmt_level(position.stop_loss_price, mid, pip_size)}"
            f" tp={_fmt_level(position.take_profit_price, mid, pip_size)}"
            f" gslo={_fmt_level(position.gslo_price, mid, pip_size)}"
        )
    await update.effective_message.reply_text("\n".join(lines))
...
async def scan_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    positional_args = [a for a in args if a.lower() != "force"]
    ...
    if positional_args:
        try:
            ...
            return
        try:
            bundle = await asyncio.to_thread(orchestrator.refresh_instrument, instrument, force=force)
        except Exception as exc:
            await _reply_command_error(
                update,
                context,
        ...
        lines = [
            ...
            f"Bundle published: {'yes' if bundle is not None else 'no'}",
        ...
        lines = [
            ...
            f"Bundles: {status.bundles_published}",
            f"Snapshots: {status.snapshots_published}",
            f"Errors: {len(status.errors)}",
        ]
...
async def smc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [
        ...
        f"Chop: {snapshot.chop.status}",
        f"Spread acceptable: {'yes' if snapshot.spread.is_acceptable else 'no'}",
    ]
    warning = _snapshot_warning_line(snapshot)
    if warning is not None:
        lines.extend(("", warning))
...
async def bias_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        instrument = normalize_command_instrument(context.args[0])
        if len(context.args) > 1:
            normalize_command_timeframe(context.args[1])
    except ValueError as exc:
        await _reply_command_error(
            update,
            context,
        ...
        return
    bundle = await _bundle_first(update, context, instrument)
    if bundle is None:
        return
    votes = ", ".join(
        f"{timeframe}:{direction}" for timeframe, direction in bundle.htf_bias.timeframe_votes.items()
    ) or "none"
    lines = [
        f"Bias {instrument}",
        f"Direction: {bundle.htf_bias.direction}",
        f"Alignment score: {bundle.htf_bias.alignment_score:.2f}",
        f"Votes: {votes}",
        f"Mixed freshness: {'yes' if bundle.mixed_freshness else 'no'}",
    ]
    await update.effective_message.reply_text("\n".join(lines))
...
async def tradeplan_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if bundle is None:
        return
    h1_snapshot = _bundle_snapshot(context, bundle, "H1")
    if h1_snapshot is None:
        await update.effective_message.reply_text(
            f"Data unavailable for {instrument} H1. Run /scan first."
        )
        return
    m15_snapshot = _bundle_snapshot(context, bundle, "M15")
    if m15_snapshot is None:
        await update.effective_message.reply_text(
            f"Data unavailable for {instrument} M15. Run /scan first."
        )
        return
    ...
    if bundle.mixed_freshness:
        warning_lines.append(
            "Warning: bundle has mixed freshness "
            f"(stalest: {bundle.stalest_timeframe or 'unknown'}, age: {int(bundle.stalest_age_seconds)}s)."
        )
    for snapshot in (h1_snapshot, m15_snapshot):
        warning = _snapshot_warning_line(snapshot)
        if warning is not None:
            warning_lines.append(f"{snapshot.timeframe}: {warning}")

    response = format_trade_plan(summary)
    if warning_lines:
...
async def order_blocks_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if warning is not None:
        lines.append(warning)
        lines.append("")
    if not snapshot.zones.order_blocks:
        lines.append("No active order blocks.")
    else:
        for block in snapshot.zones.order_blocks[:10]:
            lines.append(
                f"{block.direction} {block.lower_price:.5f}-{block.upper_price:.5f} mitigated={block.is_mitigated}"
            )
    await update.effective_message.reply_text("\n".join(lines))


async def sfp_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    session = await _require_session(update, context)
    if session is None:
        return
    if not context.args:
        await update.effective_message.reply_text(
            "Usage: /sfp <symbol> [timeframe]\n\n" + SUPPORTED_INSTRUMENTS_HELP
        )
        return
    if len(context.args) > 2:
        await update.effective_message.reply_text(
    ...
    if snapshot is None:
        return
    warning = _snapshot_warning_line(snapshot)
    if not snapshot.sfp.detected:
        lines = [f"No SFP detected for {instrument} {timeframe}."]
        if warning is not None:
            lines.extend(("", warning))
        await update.effective_message.reply_text("\n".join(lines))
        return
    lines = [
        f"SFP {instrument} {timeframe}: {snapshot.sfp.direction} sweep={snapshot.sfp.sweep_price:.5f} close={snapshot.sfp.close_price:.5f}"
    ]
    if warning is not None:
        lines.extend(("", warning))
    await update.effective_message.reply_text("\n".join(lines))
...
async def turtle_soup_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    session = await _require_session(update, context)
    if session is None:
        return
    if not context.args:
        await update.effective_message.reply_text(
            "Usage: /turtlesoup <symbol> [timeframe]\n\n" + SUPPORTED_INSTRUMENTS_HELP
        )
        return
    if len(context.args) > 2:
        await update.effective_message.reply_text(
    ...
    if snapshot is None:
        return
    warning = _snapshot_warning_line(snapshot)
    if not snapshot.turtle_soup.detected:
        lines = [f"No Turtle Soup detected for {instrument} {timeframe}."]
        if warning is not None:
            lines.extend(("", warning))
        await update.effective_message.reply_text("\n".join(lines))
        return
    lines = [
        f"Turtle Soup {instrument} {timeframe}: {snapshot.turtle_soup.direction} lookback={snapshot.turtle_soup.lookback_bars}"
    ]
    if warning is not None:
        lines.extend(("", warning))
    await update.effective_message.reply_text("\n".join(lines))
...
async def sr_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if snapshot is None:
        return
    tolerance_pips = (
        await asyncio.to_thread(_runtime_config_manager(context).effective_sr_tolerance)
    ) or 0.0
    lines = [f"Support/Resistance {instrument} {timeframe}"]
    warning = _snapshot_warning_line(snapshot)
    if warning is not None:
        lines.append(warning)
        lines.append("")
    levels = _cluster_sr_levels(snapshot, tolerance_pips=tolerance_pips)
    if not levels:
        lines.append("No clustered levels available.")
    else:
        for price, strength in levels:
            lines.append(f"{price:.5f} strength={strength}")
    await update.effective_message.reply_text("\n".join(lines))
...
async def fib_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if snapshot is None:
        return
    text = format_fib_summary(build_fib_summary(snapshot))
    warning = _snapshot_warning_line(snapshot)
    if warning is not None:
        text = f"{text}\n\n{warning}"
    await update.effective_message.reply_text(text)
...
async def config_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        lines = [
            "Runtime Config",
            f"tolerance: {snapshot.tolerance}",
            f"spread: {snapshot.spread}",
            f"chop: {snapshot.chop}",
...
async def maemfe_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    summaries = await asyncio.to_thread(_mae_mfe_summary_map, excursion_repository, open_trades)
...
async def list_price_alerts_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(format_price_alert_list(alerts))
...
async def time_alert_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        next_fire_at = (
            next_fixed_time_fire_at(local_time, now_utc=now, timezone_name=DEFAULT_TIME_ALERT_TIMEZONE)
            if kind == "at"
...
async def _bundle_first(update: Update, context: ContextTypes.DEFAULT_TYPE, instrument: str):
    market_state = context.bot_data[MARKET_STATE_KEY]
    bundle = market_state.get_bundle(instrument)
    if bundle is None:
        await update.effective_message.reply_text(f"Data unavailable for {instrument}. Try /scan first.")
        return None
    return bundle
...
def _bundle_snapshot(
    ...
    version = bundle.members.get(timeframe)
    if version is None:
        return None
    market_state = context.bot_data[MARKET_STATE_KEY]
    return market_state.get_snapshot_version(bundle.instrument, timeframe, version)
...
def _mae_mfe_summary_map(
    excursion_repository: ExcursionRepository,
    trades: list[TradeRecord],
) -> dict[str, dict[str, object] | None]:
    return {
        trade.trade_id: excursion_repository.get_mae_mfe(trade.trade_id)
        for trade in trades
    }
...
def _cluster_sr_levels(
    ...
    *,
    ...
    spec = get_instrument_spec(snapshot.instrument)
    tolerance_price = tolerance_pips * spec.pip_size
    candidates: list[float] = []
    if snapshot.structure.latest_swing_high is not None:
        candidates.append(snapshot.structure.latest_swing_high.level)
    if snapshot.structure.latest_swing_low is not None:
        candidates.append(snapshot.structure.latest_swing_low.level)
    if snapshot.smc_context.previous_high_low is not None:
        candidates.extend(
            [
                snapshot.smc_context.previous_high_low.previous_high,
                snapshot.smc_context.previous_high_low.previous_low,
            ]
        )
    candidates.extend(level.price for level in snapshot.liquidity.levels)
    ...
    for price in candidates[1:]:
        if abs(price - clusters[-1][-1]) <= tolerance_price:
...
def _build_help_text(is_admin: bool) -> str:
    ...
    sections = [
        ...
        "  /smc <symbol> [tf] - Structure, OBs, liquidity, chop",
        "  /bias <symbol> - HTF bias direction and votes",
        "  /tradeplan <symbol> - Read-only setup summary",
        "  /structure <symbol> [tf] - Recent structure breaks",
        "  /indicators <symbol> [tf] [compact|full]",
        "  /ob <symbol> [tf] - Active order blocks",
        "  /sfp <symbol> [tf] - Swing failure pattern",
        "  /turtlesoup <symbol> [tf] - Turtle soup pattern",
        "  /sr <symbol> [tf] - Support/resistance clusters",
        "  /fib <symbol> [tf] - Swing-based Fibonacci ladder",
        "  /chart <symbol> [tf] [--count N] [--mode compact|balanced|full]",
        ...
        "  /config <key> <value> - Set tolerance|spread|chop|chart|chart_mode|scan_interval|trade_push|session_alerts",
    ]
    if is_admin:
        sections.extend(
            [
                ...
                "",
                "Admin controls begin in Stage 14.",
            ]
        )
    return "\n".join(sections)
...
def main() -> int:
    """Build and run the Stage 13 Telegram bot runtime."""
