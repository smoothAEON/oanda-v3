...
class ScanOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        htf_bias_analyzer: HTFBiasAnalyzer | None = None,
        indicator_builder: Callable[[pd.DataFrame, str], object] = build_indicator_summary,
        spread_evaluator: Callable[[PriceSnapshot], object] = evaluate_spread,
        chop_evaluator: Callable[[object], object] = evaluate_chop,
        sfp_detector: Callable[[pd.DataFrame, str], object] = detect_sfp,
        turtle_soup_detector: Callable[[pd.DataFrame, str], object] = detect_turtle_soup,
        orb_detector: Callable[[pd.DataFrame, str], object] = detect_orb,
        ...
        self.htf_bias_analyzer = htf_bias_analyzer or HTFBiasAnalyzer(
            tuning=HTFBiasTuning.from_settings(self.settings),
            logger=get_logger(f"{__name__}.htf_bias"),
        )
        self.indicator_builder = indicator_builder
        self.spread_evaluator = spread_evaluator
        self.chop_evaluator = chop_evaluator
        self.sfp_detector = sfp_detector
        self.turtle_soup_detector = turtle_soup_detector
        self.orb_detector = orb_detector
    ...
    def refresh_instrument(self, instrument: str, *, force: bool = False) -> InstrumentBundle | None:
        """Refresh one instrument bundle through the full publish path."""
        ...
        try:
            ...
            bundle, snapshots_published, skipped_reason = self._scan_instrument(resolved, force=force)
        ...
        self.last_scan_status = ScanCycleStatus(
            ...
            scanned_instruments=(resolved,) if bundle is not None else (),
            snapshots_published=snapshots_published,
            bundles_published=1 if bundle is not None else 0,
            skipped_reason=None if bundle is not None else (skipped_reason or "bundle_unavailable"),
            errors=(),
        )
        return bundle

    def refresh_snapshot(self, instrument: str, timeframe: str) -> TimeframeSnapshot | None:
        """Refresh one timeframe snapshot without bundle assembly."""
        ...
        try:
            ...
            artifacts = self._build_snapshot(
                ...
                cache_only=not instrument_market_status.is_market_open,
    ...
    def _run_scan(
        ...
        *,
        ...
        for instrument in instruments:
            try:
                bundle, published, skipped_reason = self._scan_instrument(instrument, force=force)
            except Exception as exc:
                log_failure(
                    self.logger,
    ...
    def _scan_instrument(self, instrument: str, *, force: bool = False) -> tuple[InstrumentBundle | None, int, str | None]:
        ...
        for timeframe in SCAN_TIMEFRAMES:
            ...
            if artifacts is None:
                return None, 0, "market_closed_no_cache"
            snapshots[timeframe] = artifacts.snapshot
            candles_by_timeframe[timeframe] = artifacts.candles

        htf_members = [
            PinnedHTFMember(
                snapshot=snapshots[timeframe],
                candles=candles_by_timeframe[timeframe],
                source_snapshot_version=snapshots[timeframe].version,
            )
            for timeframe in HTF_TIMEFRAMES
        ]
        htf_bias = self.htf_bias_analyzer.compute(htf_members)
        calendar_events = self.calendar_provider.filter_events(currencies=relevant_currencies)
        bundle = self.market_state.assemble_bundle_from_versions(
            instrument=instrument,
            source_snapshot_versions={
                timeframe: snapshots[timeframe].version for timeframe in SCAN_TIMEFRAMES
            },
            htf_bias=htf_bias,
            calendar=calendar_events,
            calendar_version=self.calendar_provider.calendar_version,
        )
        return bundle, len(SCAN_TIMEFRAMES), None
    ...
    def _build_snapshot(
        ...
        *,
        ...
        if current_price is None:
            ...
            spread = self._timed_detector(
                "spread",
                instrument,
                timeframe,
                len(candles),
                lambda: self.spread_evaluator(current_price),
                output_counter=lambda _: 1,
            )
        chop = self._timed_detector(
            "chop",
            instrument,
            timeframe,
            len(candles),
            lambda: self.chop_evaluator(indicators),
            output_counter=lambda _: 1,
        )
        sfp = self._timed_detector(
            "sfp",
            instrument,
            timeframe,
            len(candles),
            lambda: self.sfp_detector(
                candles,
                timeframe,
                swing_length=self.settings.default_swing_length,
            ),
            output_counter=lambda result: 1 if result.detected else 0,
        )
        turtle_soup = self._timed_detector(
            "turtle_soup",
            instrument,
            timeframe,
            len(candles),
            lambda: self.turtle_soup_detector(candles, timeframe),
            output_counter=lambda result: 1 if result.detected else 0,
        )
        orb = None
        if timeframe == "M15":
            orb = self._timed_detector(
                "orb",
                instrument,
                timeframe,
                len(candles),
                lambda: self.orb_detector(candles, timeframe),
                output_counter=lambda result: 1 if result.detected else 0,
            )
    ...
    def _closed_market_spread_context(
        ...
        synthetic_price = PriceSnapshot(
            instrument=instrument,
            bid=reference_close - half_spread,
            ask=reference_close + half_spread,
            spread_price=spread_price,
            spread_pips=spec.typical_spread_pips,
            fetched_at=datetime.now(timezone.utc),
        )
        spread = self.spread_evaluator(synthetic_price)
        self.logger.info(
            ...
            max_spread_pips=spread.max_spread_pips,
        )
        return spread
