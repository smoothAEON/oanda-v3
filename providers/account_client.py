...
from core.instrument_registry import get_pip_size, normalize_instrument, validate_live_instrument
...
class OandaAccountClient:
    ...
    async def get_open_positions(self) -> list[OpenTradePosition]:
        ...
        try:
            ...
            log_failure(
                self.logger,
                "account_positions_summary_unavailable",
                exc,
                level="warning",
            )
        else:
            account_currency = summary.currency
        return [
            self._normalize_position(trade, account_currency=account_currency)
            for trade in trades
        ]
    ...
    async def get_account_summary(self) -> AccountSummary:
        ...
        return self._normalize_account_summary_payload(payload)

    def get_account_summary_sync(self) -> AccountSummary:
        """Return a typed OANDA account summary without an async boundary."""

        payload = self._request_account_summary_payload()
        return self._normalize_account_summary_payload(payload)

    def _normalize_account_summary_payload(self, payload: dict[str, Any]) -> AccountSummary:
    ...
    async def get_candles(
        ...
        if count > OANDA_MAX_CANDLE_COUNT:
    ...
    async def get_bid_ask_candles_range(
        ...
        end_utc: datetime,
        *,
        max_count_per_request: int = 5000,
    ) -> pd.DataFrame:
        """Return closed bid/ask OHLC candles across a bounded UTC time window."""

        resolved_instrument = self._resolve_instrument(instrument)
        delta = get_timeframe_delta(granularity)
        normalized_start = self._coerce_utc_datetime(start_utc)
        normalized_end = self._coerce_utc_datetime(end_utc)
        if max_count_per_request <= 0 or max_count_per_request > 5000:
            raise ValueError("max_count_per_request must be between 1 and 5000.")
        if normalized_end <= normalized_start:
            return self._empty_bid_ask_frame()

        frames: list[pd.DataFrame] = []
        chunk_span = delta * max_count_per_request
        cursor = normalized_start
        while cursor < normalized_end:
            window_end = min(normalized_end, cursor + chunk_span)
            payload = await asyncio.to_thread(
                self._request_candles_range_payload,
                resolved_instrument,
                granularity,
                cursor,
                window_end,
            ...
            frame = self._normalize_bid_ask_candle_payload(
                payload,
                instrument=resolved_instrument,
                granularity=granularity,
                allow_empty=True,
        ...
        return combined.sort_values("time", kind="mergesort").reset_index(drop=True)
    ...
    def _request_candles_payload(
        ...
        params: dict[str, object] = {
            ...
            "count": min(count + 1, OANDA_MAX_CANDLE_COUNT),
        }
    ...
    def _request_candles_range_payload(
        ...
        endpoint = endpoint_class(
            ...
            params={
                "granularity": granularity,
                "price": price_component,
                "from": self._coerce_utc_datetime(start_utc).isoformat(),
                "to": self._coerce_utc_datetime(end_utc).isoformat(),
                "includeFirst": True,
                **OANDA_CANDLE_ALIGNMENT_PARAMS,
            },
        )
        return self._request_endpoint(
            ...
            instrument=instrument,
            granularity=granularity,
            range_start_utc=self._coerce_utc_datetime(start_utc),
            range_end_utc=self._coerce_utc_datetime(end_utc),
            price_component=price_component,
        )
    ...
    def _normalize_position(
        ...
        *,
        ...
        return OpenTradePosition(
            ...
            account_currency=account_currency or self.settings.account_currency,
    ...
    @classmethod
    def _normalize_bid_ask_candle_payload(
        cls,
        payload: dict[str, Any],
        *,
        instrument: str,
        ...
        if not raw_candles:
            if allow_empty:
                return cls._empty_bid_ask_frame()
            raise RuntimeError(
                f"OANDA candle response for {instrument} {granularity} contained no candles."
            )
        ...
        for candle in raw_candles:
            ...
            rows.append(
                {
                    "time": cls._parse_oanda_time(candle.get("time")),
