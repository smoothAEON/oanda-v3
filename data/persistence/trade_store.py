...
class TradeStore:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self.alert_history = self.db.table("alert_history")
    ...
    def cancel_price_alerts_for_chat(
        ...
        *,
        ...
        """Cancel matching price alerts for one chat in a single write."""

        def write() -> list[PriceAlert]:
            assert self.price_alerts is not None
            cancelled: list[PriceAlert] = []
            status_values = {status.value for status in statuses}
            instrument_filter = None if instrument is None else instrument

            def updater(table: dict[int, Mapping[str, Any]]) -> None:
                for doc_id, record in list(table.items()):
                    if int(record.get("chat_id", -1)) != int(chat_id):
                        continue
                    if record.get("status") not in status_values:
                        continue
                    if instrument_filter is not None and record.get("instrument") != instrument_filter:
                        continue
                    payload = dict(record)
                    payload.update({"status": AlertStatus.CANCELLED.value, "armed": False, "fired_at": None})
                    validated = PriceAlert.model_validate(payload)
                    table[doc_id] = validated.model_dump(mode="json")
                    cancelled.append(validated)

            self.price_alerts._update_table(updater)
            cancelled.sort(key=lambda alert: alert.id)
            return cancelled

        return self._strict_write_operation("cancel_price_alerts_for_chat", write)

    def replace_price_alert_grid(
        self,
        *,
        chat_id: int,
        instrument: str,
        alerts: list[PriceAlert | Mapping[str, Any]],
    ) -> list[PriceAlert]:
        """Replace one chat+instrument price-alert grid in a single write."""

        get_instrument_spec(instrument)
        prepared_payloads = [
            dict(alert) if isinstance(alert, Mapping) else alert.model_dump(mode="python")
            for alert in alerts
        ]
            ...
            def updater(table: dict[int, Mapping[str, Any]]) -> None:
                ...
                for doc_id, record in list(table.items()):
                    ...
                    if record.get("instrument") != instrument:
                        continue
                    if record.get("status") != AlertStatus.PENDING.value:
                        continue
                    payload = dict(record)
                    payload.update({"status": AlertStatus.CANCELLED.value, "armed": False, "fired_at": None})
                    validated = PriceAlert.model_validate(payload)
                ...
                next_doc_id = max(table.keys(), default=0) + 1
                    ...
                    resolved.update(
                        {
                            ...
                            "armed": False,
                            "chat_id": int(chat_id),
                            "created_at": resolved.get("created_at", self._now()),
                            "fired_at": None,
                        }
                    )
                    validated = PriceAlert.model_validate(resolved)
                    created.append(validated)
                    table[self.price_alerts.document_id_class(next_doc_id)] = validated.model_dump(mode="json")
                    next_id += 1
                    next_doc_id += 1

            self.price_alerts._update_table(updater)
            created.sort(key=lambda alert: alert.id)
            return created

        return self._strict_write_operation("replace_price_alert_grid", write)
    ...
    def cancel_indicator_alerts_for_chat(
        ...
        *,
        ...
        """Cancel matching indicator alerts for one chat in a single write."""

        def write() -> list[IndicatorAlert]:
            assert self.indicator_alerts is not None
            cancelled: list[IndicatorAlert] = []
            status_values = {status.value for status in statuses}
            ...
            def updater(table: dict[int, Mapping[str, Any]]) -> None:
                for doc_id, record in list(table.items()):
                    ...
                    if record.get("status") not in status_values:
                        continue
                    if instrument is not None and record.get("instrument") != instrument:
                        continue
                    if granularity is not None and record.get("granularity") != granularity:
                        continue
                    if indicator is not None and str(record.get("indicator", "")).upper() != indicator.upper():
                        continue
                    payload = dict(record)
                    payload.update({"status": AlertStatus.CANCELLED.value, "fired_at": None})
                    validated = IndicatorAlert.model_validate(payload)
                    table[doc_id] = validated.model_dump(mode="json")
                    cancelled.append(validated)

            self.indicator_alerts._update_table(updater)
            cancelled.sort(key=lambda alert: alert.id)
            return cancelled

        return self._strict_write_operation("cancel_indicator_alerts_for_chat", write)

    def insert_alert_history(
        self,
        record: AlertHistoryRecord | Mapping[str, Any],
    ) -> AlertHistoryRecord:
        """Persist one alert-history row."""

        payload = dict(record) if isinstance(record, Mapping) else record.model_dump(mode="python")

        def write() -> AlertHistoryRecord:
            assert self.alert_history is not None
            resolved = dict(payload)
            resolved["id"] = self._resolve_alert_id_locked(resolved.get("id"), table=self.alert_history)
            validated = self._validate_alert_history(resolved)
            self.alert_history.insert(validated.model_dump(mode="json"))
            return validated

        return self._strict_write_operation("insert_alert_history", write)
    ...
    def list_alert_history(
        ...
        *,
        ...
        """Return alert-history rows newest first."""

        if limit <= 0:
            raise ValueError("limit must be a positive integer.")

        def read() -> list[AlertHistoryRecord]:
            assert self.alert_history is not None
            records = [
                self._deserialize_alert_history(record)
                for record in self.alert_history.all()
            ]
            filtered = [record for record in records if record is not None]
            if chat_id is not None:
                filtered = [record for record in filtered if record.chat_id == int(chat_id)]
            if alert_type is not None and alert_type != "all":
                filtered = [record for record in filtered if record.alert_type == alert_type]
            if instrument is not None:
                filtered = [record for record in filtered if record.instrument == instrument]
            if start_utc is not None:
                filtered = [record for record in filtered if record.triggered_at >= start_utc]
            if end_utc is not None:
                filtered = [record for record in filtered if record.triggered_at < end_utc]
            filtered.sort(key=lambda record: (record.triggered_at, record.id), reverse=True)
            return filtered[:limit]

        return self._read_operation("list_alert_history", [], read)
    ...
    def create_time_alerts(
        self,
        alerts: list[TimeAlert | Mapping[str, Any]],
    ) -> list[TimeAlert]:
        """Create multiple new time alerts atomically with fresh ids."""

        payloads = [dict(alert) if isinstance(alert, Mapping) else alert.model_dump(mode="python") for alert in alerts]

        def write() -> list[TimeAlert]:
            assert self.time_alerts is not None
            next_id = self._next_numeric_id(self.time_alerts)
            validated_alerts: list[TimeAlert] = []
            serialized_alerts: list[dict[str, Any]] = []
            for payload in payloads:
                resolved = dict(payload)
                resolved["id"] = next_id
                next_id += 1
                resolved.setdefault("status", TimeAlertStatus.ACTIVE)
                resolved.setdefault("created_at", self._now())
                resolved.setdefault("last_fired_at", None)
                validated = TimeAlert.model_validate(resolved)
                validated_alerts.append(validated)
                serialized_alerts.append(validated.model_dump(mode="json"))
            for serialized in serialized_alerts:
                self.time_alerts.insert(serialized)
            return validated_alerts

        return self._strict_write_operation("create_time_alerts", write)
    ...
    @staticmethod
    def _deserialize_alert_history(record: Any) -> AlertHistoryRecord | None:
        if record is None:
            return None
        return AlertHistoryRecord.model_validate(dict(record))
    ...
    @staticmethod
    def _validate_alert_history(
        record: AlertHistoryRecord | Mapping[str, Any],
    ) -> AlertHistoryRecord:
        return (
            record
            if isinstance(record, AlertHistoryRecord)
            else AlertHistoryRecord.model_validate(record)
        )
