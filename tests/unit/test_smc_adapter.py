...
def test_smc_adapter_zone_summary_keeps_mitigated_blocks_when_unmitigated_bucket_is_full() -> None:
    adapter = SmcAdapter(swing_length=2)
    ...
    for index in range(MAX_ACTIVE_ORDER_BLOCKS + 2):
        candidates.append(
            OrderBlockCandidate(
                ...
                upper_price=1.1100 + (index * 0.0010),
                lower_price=1.1090 + (index * 0.0010),
                created_at=pd.Timestamp(base_time + timedelta(hours=index)),
                status="ACTIVE",
                mitigated_at=None,
                last_analyzed_close=1.1000,
            )
    ...
    for index in range(2):
        candidates.append(
            OrderBlockCandidate(
                id=f"mitigated-{index}",
                instrument="EUR_USD",
                timeframe="H1",
                direction="BEARISH",
                upper_price=1.0900 - (index * 0.0010),
                lower_price=1.0890 - (index * 0.0010),
                created_at=pd.Timestamp(base_time + timedelta(hours=20 + index)),
                status="MITIGATED",
                mitigated_at=pd.Timestamp(base_time + timedelta(hours=22 + index)),
                last_analyzed_close=1.1000,
            )
        )

    summary = adapter._build_zone_summary(tuple(candidates))

    assert len(summary.order_blocks) == MAX_ACTIVE_ORDER_BLOCKS + 2
    assert sum(1 for block in summary.order_blocks if block.is_mitigated is False) == MAX_ACTIVE_ORDER_BLOCKS
    assert sum(1 for block in summary.order_blocks if block.is_mitigated is True) == 2
...
def test_smc_module_loader_surfaces_missing_package_helpfully(
    ...
    with pytest.raises(RuntimeError, match="smartmoneyconcepts is not installed"):
        _load_smc_module()
