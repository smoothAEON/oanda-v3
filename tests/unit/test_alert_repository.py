...
from core.models import AlertHistoryRecord
...
def test_alert_history_repository_filters_by_chat_and_type(tmp_path: Path) -> None:
    store = TradeStore(db_path=tmp_path / "alert_history.json")
    repository = AlertRepository(store=store)
    try:
        repository.insert_alert_history(
            AlertHistoryRecord(
                id=1,
                alert_type="price",
                alert_id=10,
                ...
                trigger_value=3050.0,
                alert_snapshot={"direction": "above"},
                trigger_context={"bid": 3049.8},
            )
        )
        repository.insert_alert_history(
            AlertHistoryRecord(
        ...
        rows = repository.list_alert_history(chat_id=321, alert_type="price", limit=10)

        assert len(rows) == 1
        assert rows[0].chat_id == 321
        assert rows[0].alert_type == "price"
    finally:
        store.close()
