...
def test_next_fixed_datetime_fire_at_interprets_exact_sgt_datetime() -> None:
    fire_at = next_fixed_datetime_fire_at("2026-03-29 09:30", now_utc=BASE_TIME)

    assert fire_at == datetime(2026, 3, 29, 1, 30, tzinfo=timezone.utc)


def test_next_fixed_datetime_fire_at_rejects_past_datetime() -> None:
    with pytest.raises(ValueError, match="future date and time"):
        next_fixed_datetime_fire_at("2026-03-28 09:30", now_utc=BASE_TIME)
...
def test_time_alert_engine_marks_one_shot_alert_completed_and_dispatches(tmp_path: Path) -> None:
    ...
    try:
        ...
        history = repository.list_alert_history(chat_id=123, alert_type="time", limit=10)
...
def test_time_alert_notification_failure_leaves_alert_due(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert repository.list_alert_history(chat_id=123, alert_type="time", limit=10) == []
    finally:
        store.close()
