    ...
    return pd.DataFrame(
        {
            ...
            ("Adj Close", "CL=F"): [68.10, 69.25],
            ("Adj Close", "^GSPC"): [5740.0, 5785.5],
            ("Adj Close", "^TNX"): [4.18, 4.24],
            ("Close", "^VIX"): [27.44, 31.05],
            ("Close", "DX-Y.NYB"): [99.90, 100.15],
            ("Close", "CL=F"): [68.10, 69.25],
            ("Close", "^GSPC"): [5740.0, 5785.5],
            ("Close", "^TNX"): [4.18, 4.24],
        },
        index=index,
    )
...
def test_macro_context_refresh_reads_bounded_symbols() -> None:
    ...
    assert calls == [("^VIX", "DX-Y.NYB", "CL=F", "^GSPC", "^TNX")]
    assert status.used_cached is False
    assert status.last_error is None
    assert status.vix.value == 31.05
    assert status.dxy.value == 100.15
    assert status.cl.value == 69.25
    assert status.spx.value == 5785.5
    assert status.us10y.value == 4.24
...
def test_macro_context_returns_cached_status_within_refresh_window() -> None:
    ...
    assert second.cl.value == first.cl.value
    assert second.spx.value == first.spx.value
    assert second.us10y.value == first.us10y.value
...
def test_macro_context_falls_back_to_last_good_snapshot_on_refresh_failure() -> None:
    ...
    assert degraded.cl.value == initial.cl.value
    assert degraded.spx.value == initial.spx.value
    assert degraded.us10y.value == initial.us10y.value
...
def test_macro_context_reports_unavailable_when_first_refresh_fails() -> None:
    ...
    assert status.cl.value is None
    assert status.spx.value is None
    assert status.us10y.value is None
