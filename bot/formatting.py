...
def format_orders_grouped(
    ...
    if entry_orders:
        ...
        for order in entry_orders:
            ...
            pip_size = _resolve_pip_size(instrument, default=1.0) if instrument != "N/A" and mid is not None else 1.0
    ...
    if risk_orders:
        ...
        for order in risk_orders:
            ...
            pip_size = _resolve_pip_size(instrument, default=1.0) if instrument and mid is not None else 1.0
            dist_str = _pip_dist(order.price, mid, pip_size) if mid is not None else ""
            lines.append(
    ...
    if not alerts:
        ...
        for alert in alerts:
            if alert.session_name is not None:
                ...
                descriptor = (
                    f"at {alert.local_time}"
                    if is_time_alert_local_datetime_text(alert.local_time)
                    else f"at {alert.local_time} {alert.schedule}"
                )
            next_fire = (
                "unknown"
...
def format_vwap_summary(result: VwapReadResult) -> str:
    """Return the canonical `/vwap` response."""

    position_text = "AT VWAP" if result.price_position == "at" else f"{result.price_position.upper()} VWAP"
    lines = [
        f"VWAP {result.instrument} {result.timeframe}",
        f"Anchor: {result.anchor} ({result.anchor_name}) from {_format_timestamp(result.anchor_start)}",
        f"As of: {_format_timestamp(result.last_completed_candle)}",
        f"Close: {result.reference_close:.5f}",
        f"VWAP: {result.vwap:.5f}",
        f"Position: {position_text}",
        f"Distance: {result.distance_price:+.5f} ({_signed_pips(result.distance_pips)})",
        f"Source: {result.source}  Volume: {result.volume_type}",
    ]
    if result.bands:
        lines.append("")
        lines.append("Bands:")
        for band in result.bands:
            lines.append(f"{band.deviation:g}σ  {band.lower:.5f} - {band.upper:.5f}")
    lines.extend(("", f"Caveat: {result.caveat}"))
    return "\n".join(lines)
    ...
    if not trades:
        ...
        for trade in trades:
            ...
            if current_price is not None:
                ...
                pl_text = f"  current={current_price:.5f}  P/L: {_signed_pips(pl_pips)}"
...
def format_maemfe_detail(
    ...
    *,
    ...
    mae_pips = float(summary.get("mae_pips", 0.0))
    mfe_pips = float(summary.get("mfe_pips", 0.0))
    mae_at = summary.get("mae_at")
    mfe_at = summary.get("mfe_at")
    if not isinstance(mae_at, datetime) or not isinstance(mfe_at, datetime):
        raise ValueError("MAE/MFE detail summary requires datetime timestamps.")
    ...
    lines.extend([
        "",
        f"MAE (worst):  {_signed_pips(-mae_pips)}  (at {mae_at.strftime('%H:%M UTC')})",
        f"MFE (best):   {_signed_pips(mfe_pips)}  (at {mfe_at.strftime('%H:%M UTC')})",
        "",
...
def _compute_pl_pips(trade: TradeRecord, current_price: float) -> float:
    ...
    try:
        pip_size = _resolve_pip_size(trade.instrument, default=0.0001)
...
def _format_trade_account_pnl(
    trade: TradeRecord,
    *,
    default_account_currency: str,
) -> str:
    if trade.state == TradeState.OPEN or trade.account_pnl is None:
        return "n/a"
    currency = trade.account_currency or default_account_currency
    return _format_money(trade.account_pnl, currency)


def _format_trade_history_summary_headline(
    page: TradeHistoryPage,
    *,
    end_local: datetime,
) -> str:
    if page.page_date_local is not None and page.page_date_summary is not None:
        return (
            f"P&L ({page.page_date_local.isoformat()}): "
            f"{_format_decimal_money(page.page_date_summary.net_realized_pl, signed=True)}"
        )
    summary_scope = "window"
    if page.summary.start_local.date() == end_local.date():
        summary_scope = end_local.date().isoformat()
    return f"P&L ({summary_scope}): {_format_decimal_money(page.summary.net_realized_pl, signed=True)}"


def _format_trade_history_navigation(page: TradeHistoryPage) -> list[str]:
    if page.total_pages <= 1:
        return []

    lines: list[str] = []
    if page.page > 1:
        lines.append(f"Prev: {_format_trade_history_command(page, page.page - 1)}")
    if page.page < page.total_pages:
        lines.append(f"Next: {_format_trade_history_command(page, page.page + 1)}")
    return lines


def _format_trade_history_command(page: TradeHistoryPage, target_page: int) -> str:
    tokens = ["/tradehistory", page.period, page.view]
    if page.instrument is not None:
        tokens.append(page.instrument)
    tokens.append(str(target_page))
    return " ".join(tokens)
...
def format_journal_list(
    ...
    *,
    ...
    if not trades:
        ...
        for trade in trades[:10]:
            pips = "n/a" if trade.state == TradeState.OPEN or trade.pips is None else _signed_pips(trade.pips)
            money = _format_trade_account_pnl(trade, default_account_currency=account_currency)
            lines.append(
                f"#{trade.trade_id}  {trade.instrument}  {trade.direction:<5}  {trade.state.value:<6}  "
                f"{pips:<11}  {money:<12}  {trade.opened_at.date().isoformat()}"
            )
    lines.extend(("", f"Filters: {filter_summary}  |  /journal <id> for full detail"))
    return "\n".join(lines)
...
def format_journal_detail(
    ...
    *,
    ...
    if trade.state == TradeState.CLOSED:
        pnl_value = trade.account_pnl if trade.account_pnl is not None else 0.0
        resolved_account_currency = trade.account_currency or account_currency
        lines.append(
            f"P&L:          {_signed_pips(trade.pips or 0.0)}  |  {_format_money(pnl_value, resolved_account_currency)}"
        )
        if trade.closed_at is not None:
            lines.append(f"Duration:     {_format_duration(trade.opened_at, trade.closed_at)}")
        lines.append(f"Opened:       {_format_timestamp(trade.opened_at)}")
...
def format_trade_history_page(page: TradeHistoryPage) -> str:
    ...
    lines = [
        f"Trade History - {page.period.upper()} - {page.view.upper()} - {instrument_label}",
        _format_trade_history_summary_headline(page, end_local=end_local),
        f"Window: {page.window_start_local.strftime('%Y-%m-%d %H:%M %Z')} -> {end_local.strftime('%Y-%m-%d %H:%M %Z')}",
        f"Trades: {page.total_rows}",
        f"Gross Realized PnL: {_format_decimal_money(page.summary.gross_realized_pl, signed=True)}",
        f"Financing: {_format_decimal_money(page.summary.financing, signed=True)}",
    ...
    lines.extend(("", f"Page {page.page}/{page.total_pages}"))
    navigation_lines = _format_trade_history_navigation(page)
    if navigation_lines:
...
def format_positions(
    ...
    *,
    ...
    """Return Telegram-friendly `/positions` cards."""
    ...
    for position in positions:
        instrument = str(getattr(position, "instrument", "N/A"))
        mid = mids.get(instrument)
        pip_size = _resolve_pip_size(instrument, default=1.0) if instrument != "N/A" else 1.0
        currency = getattr(position, "account_currency", None)
        lines.extend(
            [
                "",
                f"#{position.trade_id}  {instrument}  {position.direction}",
                f"Units:           {abs(float(position.units)):.2f}",
                f"Unrealized P/L:  {_format_position_money(getattr(position, 'unrealized_pl', None), currency)}",
                f"Current Mid:     {_format_position_price(mid)}",
                f"Entry:           {_format_position_level(position.open_price, mid, pip_size)}",
                f"SL:              {_format_position_level(position.stop_loss_price, mid, pip_size)}",
                f"TP:              {_format_position_level(position.take_profit_price, mid, pip_size)}",
                f"GSLO:            {_format_position_level(position.gslo_price, mid, pip_size)}",
                f"Opened:          {_format_timestamp(position.opened_at)}",
            ]
        )
    return "\n".join(lines)


def _resolve_pip_size(instrument: str, *, default: float) -> float:
    try:
...
def _format_position_level(level: float | None, mid: float | None, pip_size: float) -> str:
    if level is None:
        return "None"
    value = f"{level:.5f}"
    if mid is None:
        return value
    return f"{value} {_pip_dist(level, mid, pip_size)}"


def _format_position_money(value: float | None, currency: str | None) -> str:
    if value is None:
        return "n/a"
    if currency is None:
        return f"{value:+.2f}"
...
def _format_position_price(value: float | None) -> str:
    if value is None:
        return "Unavailable"
    return f"{value:.5f}"
